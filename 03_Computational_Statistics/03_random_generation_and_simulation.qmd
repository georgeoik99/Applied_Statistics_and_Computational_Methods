---
title: "Random-Variable Generation and Simulation"
subtitle: "Built-in generators, inversion, rejection, and mixtures"
...
This notebook moves from R's built-in distribution functions to three general simulation strategies: inverse transform, rejection sampling, and composition. Every experiment is reproducible.
...
The same naming convention applies to `unif`, `exp`, `gamma`, `binom`, `pois`, and many other distributions.
...
Small differences are Monte Carlo error, not model failure. They shrink as the simulation size increases.

## Inverse-transform simulation

If $U\sim U(0,1)$ and $F$ is a continuous CDF, then $F^{-1}(U)$ has CDF $F$. For an exponential distribution with rate $\lambda$,
...
Let the target be $\operatorname{Gamma}(\alpha,\beta)$ with shape $\alpha>1$ and rate $\beta$. Use an exponential proposal with rate $0<\lambda<\beta$.
...
A mixture can be simulated in two steps: first choose a component, then draw from that component.
...
The course data record individual survival outcomes. A simple binomial simulation uses the observed survival proportion as a plug-in probability; a hierarchical extension can also let the number of trials be random.
...
Simulation reproduces a specified model; it does not validate the assumptions used to specify that model.
