---
title: "Leukemia Gene Expression and Multiple Testing"
subtitle: "PCA, gene-wise t-tests, FWER, FDR, and pFDR in R"
format:
  gfm:
    toc: true
...
This notebook compares gene expression between the ALL and non-leukemia groups. It follows the original assignment workflow while correcting the order of the q-value calculations, the rejection counts, and the distinction between FDR and pFDR.
...
The analysis contains 20,172 genes and 24 samples: 12 ALL and 12 NoL observations.
...
The sample-level projection is the relevant view for comparing patient groups. The gene-level projection instead summarizes relationships among genes across the 24 samples.
...
The assignment assumes independent samples, approximate normality, and equal group variances.
...
The horizontal line shows the estimated proportion of true null hypotheses on the density scale.
...
`sum(rejected)` counts the discoveries directly. Applying a second comparison such as `sum(rejected < alpha)` would instead count the `FALSE` values.

## Visualization using BH-FDR decisions

Question 6 asks for visualization under FDR control, so the following plots use the Benjamini-Hochberg decisions rather than q-value decisions.
...
This final separation is exploratory. The same group labels were used first to select the genes and then to construct the PCA, so the plot is not an independent validation of classification performance.
